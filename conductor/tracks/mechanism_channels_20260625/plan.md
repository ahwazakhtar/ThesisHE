...
All completed tasks landed in commit **1047ec4** (Session 8, 2026-07-01).
...
- [x] **Industry composition (Channel 2):** 1047ec4 ACS **C24030** (existing key), male lines 003–027 / female 029–053. `Ag_Emp_Share = (004+030)/001` (ag/forestry/fishing/hunting only); `ClimateExposed_NonFarm_Share = (mining 005/031 + construction 006/032 + manufacturing 007/033 + transport&warehousing 011/037 + utilities 012/038)/001`. ACS5 county 2011+.
- [x] **Migration (Channel 7):** 1047ec4 IRS SOI county-to-county, keyless `https://www.irs.gov/pub/irs-soi/countyinflow<TOK>.csv` / `countyoutflow<TOK>.csv`, tokens 1112…2021 (2011→2021). Cols `y1/y2_statefips,countyfips,state,countyname,n1,n2,agi`; summary rows state FIPS 96/97/98 (97-000=total US migration); non-migrant = origin==destination; suppression = -1; agi in $000s. 2018-19+ dropped whole-state rows + raised threshold to 20.
- [x] **Health utilization/spending (Channel 3):** 1047ec4 CMS Medicare Geographic Variation county PUF — resolve current CSV via `data.cms.gov/data.json`; 2014–2023; filter `BENE_GEO_LVL==County` & `BENE_AGE_LVL==All`.
- [x] **Energy burden (Channel 4):** 1047ec4 DOE LEAD (NREL/OpenEI) per-state ZIPs `data.openei.org/files/6219/{ST}-2022-LEAD-data.zip` (hyphens); county AMI CSV inside; single 2022 cross-sectional vintage → time-invariant; energy burden DERIVED household-weighted.
...
- [x] **Task: Ag-dependence moderator.** 1047ec4 `download_county_agriculture.R` (USDA CSV + BEA CAINC5N 81/35) + `process_county_agriculture.R` → `Data/intermediate_ag_dependence.rds`. Built 2026-07-01: 3,183 counties; **444 farming-dependent** (USDA non-overlapping type, matches ERS headline; `Ag_Dependent_Flag`=507 broader), `Farm_Earnings_Share` (baseline-avg 2001–2010) for 3,116 counties (median 2%, max 73%), balanced terciles.
- [x] **Task: Climate-exposed industry-share moderator.** 1047ec4 `download_county_industry.R` (ACS C24030) + `process_county_industry.R` → `Data/intermediate_industry_composition.rds`. Built: 41,869 county-years (2011–2023), `ClimateExposed_NonFarm_Share` baseline median 25% (5–67%), `Ag_Emp_Share` median 3%, `ClimateExposed_Tercile`, all shares ∈[0,1]. THE variable separating the labor channel from agriculture.
- [x] **Task: County migration (IRS SOI).** 1047ec4 `download_county_migration.R` + `process_county_migration.R` → `Data/intermediate_migration.rds`. Built: 29,422 county-years (2012–2021), net-migration rate ~0-centered. CAVEAT: 2021 coverage drops to 1,150 counties (2020-21 disclosure/COVID suppression).
- [x] **Task: Medicare spending/utilization (CMS).** 1047ec4 `download_county_medicare.R` (resolves URL via data.json) + `process_county_medicare.R` → `Data/intermediate_medicare_spending.rds`. Built: 31,949 county-years (**2014–2023** — PUF starts 2014), std payment PC median $9,825, ER visits/1000 median 639. (Fix: select-only read via `colClasses="NULL"` — full-char read segfaults the 58MB/246-col file.)
- [x] **Task: Energy burden (DOE LEAD).** 1047ec4 `download_county_energy.R` (51 state ZIPs → county AMI CSV, discard ZIP) + `process_county_energy.R` → `Data/intermediate_energy_burden.rds`. Built 2026-07-01: 3,144 counties; overall burden median 3.3%, **low-income (≤80% AMI) median 8.7%** (~2.6× — the distributional signature that maps to high-SVI amplification). TIME-INVARIANT (single 2022 vintage).
- [x] **Task: Tests** 1047ec4 `Code/tests/test_mechanism_data.R` (testthat) — all 6 pass: FIPS zero-pad idiom; per-intermediate schema, 5-digit FIPS, no dup keys, share ∈[0,1], tercile sanity, 444 farm-dependent, low-income>overall energy burden.
- [x] **Data-gate validation:** 1047ec4 `Code/diagnostics/check_mechanism_merge.R` — all 5 intermediates merge onto county master at 97.3–98.5%. Unmatched ~2% are known FIPS boundary cases (CT 2022 planning regions `091xx`↔old `090xx`; AK renames `02280/02201`; CO `089xx`; DC/Baltimore). Log in build_logs/.
...
- [x] **Task 2a: Agricultural-channel bound (PRIMARY).** 1047ec4 `Code/run_mechanism_agriculture.R` →
...
- [x] **Task 2b: Labor-channel test.** 1047ec4 Same script, moderator `ClimateExposed_NonFarm_Share`
...
- [x] **Task 2c: Medicare morbidity/utilization regressions (Channel 3).** 1047ec4 `Code/run_mechanism_medicare.R`
...
- [x] **Task 2d: Energy-burden distributional test (Channel 4).** 1047ec4 `Code/run_mechanism_secondary.R` →
...
- [x] **Task 2e: Migration selection check (Channel 7 caveat).** 1047ec4 Same script →
...
- [x] **Task 2f: Verdict.** 1047ec4 `Analysis/mechanism/mechanism_verdict.md` + 3 forest plots
...
- [x] **Task: Map the channel set.** 1047ec4 `Analysis/mechanism/mechanism_channels.md` (lit-review grounded).
...
- [x] **Task: Document shocks-as-distributional-draws.** 1047ec4 Technical-note **§1.2**: shocks are z-score deviations anchored to the frozen **1990–2000** baseline (per-county mean/SD) — departures from each county's own historical distribution, addressing "unanticipatable given historical averages."
- [x] **Task: Document multiple-shock handling.** 1047ec4 Technical-note **§1.3**: distributed lags + compound/`Any_Shock` + cumulative-dose (`cumulative_dose.R`, §3.6) absorb shock multiplicity.

## Phase 5: Synthesis & integration

- [x] **Task: Mechanisms subsection.** 1047ec4 Technical-note **§6 Mechanisms** (separability test + one-paragraph reviewer answer) and `Analysis/county_analysis_summary.md` **§5**; ties the agricultural bound + other-channel map together with the in-panel verdict.
