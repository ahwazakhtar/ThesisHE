...
- [ ] **Task: Econometric Soundness Remediation (Pre-Event-Study Gate)**
    - [x] **Deferred → completed via `thesis_completion_20260704` 2.2 (`fca5643`, 2026-07-13):** Enforce one-row-per-county-year (`fips_code`, `Year`) in county master by resolving split-county multi-rating-area premium joins. [Unweighted-mean rule across RAs for premium columns; build-time uniqueness + constancy assertions; defense documentation in `Analysis/county_dedup_integrity.md`; 2012 DiD ATTs identical, headline debt cells <0.08 SE.]
