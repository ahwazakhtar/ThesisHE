...
- County drought block multicollinearity: `run_county_analysis.R` primary specs use PDSI only (`drought_vars_primary`: pdsi_val + Lag1/Lag2) to avoid VIF inflation from near-collinear PDSI/PHDI/PMDI. Full 9-variable block retained as `drought_vars_robust_full` for optional robustness specs. VIF diagnostics not yet added to county script (state script has them in `Analysis/vif_diagnostics.txt`).
