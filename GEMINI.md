...
*   **Writing/dissemination (Jun 2026):** three-essay thesis abstracts + umbrella (`Text/thesis_paper_abstracts.md`), a conference abstract (`Text/conference_abstract.*`), and a committee update deck (`Text/committee_presentation_20260615.tex`, + a hyperlinked-appendix detailed variant). **(Jul 2026)** Mechanism findings woven into the three-essay abstracts; external-reader response in NBER style (`Text/reviewer_response_mechanisms_nber.md`, `_email.md`) plus a self-contained §6 Mechanisms section (`Text/mechanisms_section.md`). A reusable NBER economist writing-style skill (reverse-engineered from `Text/w33491.pdf`) lives in `.claude/skills/nber-economist-writing-style/`.
*   **Hospital Supply-Side Integration:** **implemented (Phases 1–5).** Hospital-year panel (NASHP HCT) woven through the three papers with provider heterogeneity; only the Conductor verification gate is open. Track `hospital_supply_side_20260615`.
*   **Mechanisms — Agricultural Channel & Beyond:** **implemented (Phases 0–6; verification gate open).** External-reader response bounding the agricultural income channel. Five new county data sources (USDA/BEA ag-dependence, ACS C24030 climate-exposed non-farm employment, IRS SOI migration, CMS Medicare Geographic Variation, DOE LEAD energy burden). Verdict: **agriculture is one channel, not the channel** — a Medicare-measured morbidity/utilization channel (heat/cold/AQI → spending & ED visits) and a broad labor-exposure channel are robust and non-agricultural; energy burden is a distinct distributional channel; the drought effect is event-specific and partly population selection. See `Analysis/mechanism/` (`mechanism_channels.md`, `mechanism_verdict.md`) and technical-note §1.2/§1.3/§6.
