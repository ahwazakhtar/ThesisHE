...
# §5 Main income result — WRITE FIRST

### §5 ¶1 — The estimate
1. Design recap in one clause: 139 first-onset counties vs 2,534 never-exposed, county+year FE, ITT of the 2012 onset.
2. Headline number: real per-capita income fell **$1,311** (analytic p=0.027).
3. Anchor: against a mean county PCPI of $46,269, that is **≈2.8%** of per-capita income.
4. Second anchor (population): applies to the residents of the 139 treated counties in Georgia, the Mountain West, and the Plains — an event-and-geography-specific ATT, not a national parameter.
→ but with 17 treated states, analytic p-values are not the right yardstick — inference next.

### §5 ¶2 — Robust inference
...
4. Interpretation: the estimate survives the two inference procedures designed for exactly this cluster structure (ties back to the §3 AAIW paragraph).
→ inference is one threat; composition is another — the doubly-robust check.

### §5 ¶3 — Doubly-robust check
...
2. DRDID (covariate-adjusted, doubly robust): ATT **−$1,451** (SE 515, CI [−2,461, −441]).
3. Reading: conditioning on observable composition **strengthens** the estimate rather than eroding it — the raw 2×2 is, if anything, conservative.
→ level effect established; what does the response look like over event time?

### §5 ¶4 — Dynamics
1. Event-time profile from the frontier doubly-robust event study (E1-F3): describe onset timing, trough, and post-event path as the figure shows them.
2. Point out: no pre-event divergence visible in event time (full two-decade test in §6).
3. One sentence only: whether the loss reverses, scars, or compounds is the subject of Essay 2 — this essay establishes incidence.
→ how big is this, in context?

### §5 ¶5 — Magnitude in context
1. Per-county scale: −$1,311 per resident × county population = the county-level income bill (leave as reader arithmetic or compute for the median treated county).
2. Literature yardstick: compare with drought/temperature income effects in the climate-economy literature **[CITE: Deschênes–Greenstone; Deryugina–Hsiang-style aggregate estimates]** — same order of magnitude for an extreme event.
3. Window robustness (advisor item 1.3): extending the estimation window back to 1990 leaves the drought–income relationship stable (distributed-lag PDSI coefficient −$99 to −$132 across 1990/2000/2011 start years) with precision improving; the full window even makes the contemporaneous term significant (−$149, p=5e-6).
4. Takeaway of that: the 2011–2023 primary window is a conservative choice, not a favorable one; adding 2024 changes nothing.
→ the estimate is large and stable — now the identification case in full.

---

# §6 Identification and falsification

### §6 ¶1 — Two-decade pre-trend
...
### §6 ¶2 — Leave-one-treated-state-out
...
### §6 ¶4 — Spatial robustness (spillovers + Conley)
...
### §6 ¶6 — What this design does not establish
...
# §7 External validity and pooled cohorts

### §7 ¶1 — The question and the tool
...
### §7 ¶4 — Employment: the fragile secondary
...
# §8 Direct Medicare utilization evidence

### §8 ¶1 — Why Medicare
1. The income result shows economic damage; it does not show a health channel. Rather than infer one, measure one.
2. Medicare (CMS Geographic Variation): administrative, near-universal for 65+/disabled, standardized payments strip price variation — direct observation of utilization free of insurance-composition churn.
3. Framing sentence (binding): this is **parallel, direct evidence of a separate morbidity channel** — it does not mediate the drought-income result and is not extended to working-age households.
...
### §8 ¶6 — The labor-exposure margin (mechanism-supporting)
...
# §9 Financial-ledger responses: debt and ACA premiums
...
### §9 ¶3 — Debt: drought **[DECIDE — recommended resolution below]**
...
### §9 ¶7 — Premiums: equivalence bounds
...
### §9 ¶9 — Institutional interpretation
...
### §10 ¶1 — What the three results jointly say
...
### §10 ¶4 — What would change the conclusions
...
### §11 ¶1 — Findings restated
1. Question restated in one sentence (costs + which institutions record/price them).
2. Three findings, one sentence each, each with its one-clause qualifier: −$1,311 (≈2.8% of PCPI; the 2012 event, robust inference, flat two-decade pre-trend); Medicare +$112/+$177 per beneficiary and +8–10 ED visits/1,000 (65+/disabled; direct, non-agricultural); no coherent ACA pass-through (sign-unstable across levels; drought bounded at 50–79% of morbidity costs).
→ the lesson.

### §11 ¶2 — Institutional lesson + baton
...
# §4 The 2012 drought design

### §4 ¶1 — Why 2012, why first onset
...
### §4 ¶4 — Identifying assumption and threats
...
### §1 ¶4 — Result 1 (income)
1. −$1,311 per capita, ≈2.8% of the $46,269 mean (analytic p=0.027; WCB p=0.036; RI p=0.0075).
2. Robustness clause: flat 1990–2011 pre-trend (−$69/yr, p=0.44); strengthens to −$1,451 under DRDID.
3. Same-paragraph qualifier: event-specific — the pooled multi-cohort estimator is null at onset (−$324, SE 276) and long-run; the paper separates these estimands explicitly.

### §1 ¶5 — Result 2 (Medicare)
1. Separate and parallel — not a mediator: heat raises standardized Medicare spending $112 now, $177 next year (1.1%/1.7% of the $10,359 base) and ED visits ~8–10/1,000 (1.2–1.5% of 629); cold and AQI effects at lags; reproduces Deryugina et al. in-panel.
2. Population qualifier in the same breath: 65+/disabled, 2014–2023; non-agricultural (Drought×Ag null).

### §1 ¶6 — Result 3 (ledgers)
1. Debt: cold +1.35 pp at one-year lag (state level; county +1.2 pp) — recorded, but the ledger is measurement-fragile (three filters).
2. Premiums: no coherent local pass-through — sign flips across county/RA/state; within-state responses ≤5–8% of the mean premium; the data rule out morbidity-scale pass-through **for drought** (50–79% of the benchmark excluded); heat/cold only loosely bounded.
3. Corollary: 92–99% of the debt response survives premium adjustment — there is no premium channel to travel through.

### §1 ¶7 — Contributions
1. Causal incidence: among the sharpest event-based estimates of a drought's local income cost, with few-cluster-robust inference and a two-decade pre-trend **[CITE the 2-3 nearest papers]**.
2. Direct morbidity: administrative utilization responses measured in the same panel as the economic outcomes — the health channel by measurement, not inference.
3. Institutional visibility: a new fact about *recording* — regulated ACA pricing shows no coherent local response and bureau debt records selectively; which ledger you read determines whether you see the cost.

### §1 ¶8 — Roadmap
1. One or two sentences; section order.

---

# Abstract — WRITE VERY LAST

String, in order, exactly these units (target 180–250 words):
1. Question (one sentence — costs and which institutions record/price them).
2. Design (2012 first-onset drought vs never-exposed; harmonized panel clause).
3. Income result + inference + event-specific qualifier (one sentence, one parenthetical).
4. Medicare sentence ($112/$177; ED +8–10/1,000; 65+/disabled; direct/parallel).
5. Ledger sentence (debt lagged but measurement-fragile; premiums: no coherent pass-through, drought tightly bounded).
6. Contribution sentence (locating climate costs in lagged local outcomes and identifying which ledgers record or price them).
