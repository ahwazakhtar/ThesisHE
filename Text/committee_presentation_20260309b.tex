...
\begin{frame}{Empirical Specification}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.44\textwidth}
      \textbf{\tealtext{Outcome Variables}}\\[0.2em]
      \scriptsize
      \textit{Household / individual:}
      \begin{itemize}\itemsep0.03em
        \item Employer premium contribution (\$/yr) \graytext{MEPS-IC}
        \item Benchmark Silver premium (\$2023) \graytext{HIX}
        \item Medical debt share (\% with debt) \graytext{Urban Inst.}
        \item Medical debt median (\$2023) \graytext{Urban Inst.}
        \item Median HH income (\$2023) \graytext{ACS}
      \end{itemize}
      \vspace{0.12em}
      \textit{Hospital / system:}
      \begin{itemize}\itemsep0.03em
        \item Hospital bad debt per capita \graytext{NASHP}
        \item Total per-capita health expenditure \graytext{CMS}
        \item Medicaid per enrollee \graytext{CMS}
        \item Medicare per enrollee \graytext{CMS}
      \end{itemize}
      \vspace{0.12em}
      \textit{Macro economic:}
      \begin{itemize}\itemsep0.03em
        \item Per-capita personal income (\$2023) \graytext{BEA}
        \item Civilian employment (persons) \graytext{ACS}
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      ...
      \begin{tabular}{lrl}
        ...
        Pct Unhealthy (lag 0--2) & $+97.7$  & \emphcolor{**}  \\
        \bottomrule
      \end{tabular}
      \vspace{0.2em}
  ...
  \begin{columns}[T]
    \begin{column}{0.58\textwidth}
      ...
      \begin{tikzpicture}[scale=0.80,
        ...
        \node[box] (prem)  at (5.2, 1.1)  {$+\$86$ premium\\(Drought+Cold, $p{<}0.05$)};
        \node[box] (dshare) at (5.2,-0.1) {$+1.4$ pp debt share\\(Cold lag 1, $p{<}0.05$)};
        \node[box] (dmed)  at (5.2,-1.2)  {$+\$59$ debt median\\(High HDD, $p{<}0.05$)};
        \node[box] (exp)   at (5.2,-2.4)  {$+\$205$ total hlth exp\\(Cold, $p{<}0.05$)};

        % Outcome nodes — public payer channel
        \node[box] (mc1)   at (5.2,-3.5)  {$+\$145/+\$137$ Medicare\\(CDD lag 1/2, $p{<}0.01$)};
        \node[box] (mc2)   at (5.2,-4.6)  {$-\$110/-\$193$ Medicare\\(HDD lag 1/2, $p{<}0.10/0.01$)};
        \node[box] (mcd)   at (5.2,-5.7)  {$-\$525$ Medicaid/enrol\\(Drought lag 1, $p{<}0.01$)};
        ...
        % Channel labels
        \node[font=\scriptsize\itshape, text=WarmGray, anchor=west] at (7.35,-0.9) {Household};
        \node[font=\scriptsize\itshape, text=WarmGray, anchor=west] at (7.35,-4.2) {Public Payer};
      \end{tikzpicture}
      \end{center}
    \end{column}
    \begin{column}{0.38\textwidth}
      ...
      \begin{itemize}\itemsep0.18em
        ...
        \item Lagged debt accumulation (share + median)
        \item Boosts total system expenditure
        \item Reduces Medicare spending (HDD lag)
      \end{itemize}
      \vspace{0.35em}
      \textbf{Heat channel (CDD):}
      \begin{itemize}\itemsep0.18em
        \item Raises Medicare with 1--2 yr lag
        \item Consistent with elderly heat utilisation
      \end{itemize}
      \vspace{0.35em}
      \textbf{Drought:}
      \begin{itemize}\itemsep0.18em
        \item Persistently cuts Medicaid spending
        \item Raises premiums contemporaneously
      \end{itemize}
      \vspace{0.35em}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.47\textwidth}
      ...
      \begin{itemize}\itemsep0.28em
        \item \emphcolor{Cold (HDD)} raises premiums on impact across all specs and both cluster levels
        \item \emphcolor{Heat} contemporaneously reduces premiums (RA-cluster: $-10.9^{**}$) but raises them 1--2 years later
        \item RA-clustering consistently tightens SEs; results survive
...
\begin{frame}{County: Heat Depresses Income; Drought Suppresses Employment}
  \framesubtitle{Median HH Income and Per-Capita Income (\$2023); Civilian Employment (persons)}
  \small
  \begin{columns}[T]
    \begin{column}{0.47\textwidth}
    ...
    \begin{column}{0.47\textwidth}
      ...
      \begin{itemize}\itemsep0.2em
        \item \emphcolor{Heat consistently reduces HH income} in pop-weighted specs across all lags
        \item \emphcolor{Cold suppresses employment} unweighted; no PW signal
        \item \emphcolor{Drought severely reduces employment} in pop-weighted (urban) counties
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.47\textwidth}
      \begin{itemize}\itemsep0.35em
        ...
        \item \emphcolor{PDSI (drought)} shows lagged positive effect: drier conditions slowly push bad debt up, confirmed in RA-level robustness
        \item \emphcolor{Uninsured Rate} is the dominant control ($+151^{***}$ unweighted) — structural coverage gap drives hospital write-offs
      \end{itemize}
      \vspace{0.3em}
      \graytext{\scriptsize $\sim$23\% of county-years missing (no hospital\\reports); one negative outlier winsorized.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Model Diagnostics and Robustness}
  ...
  \begin{columns}[T]
    \begin{column}{0.47\textwidth}
      ...
      \begin{itemize}\itemsep0.22em
        ...
        \item CDD/HDD: national p80, 1990--2000 baseline\\(corrected from within-state all-years)
    ...
    \begin{column}{0.47\textwidth}
      ...
      \begin{itemize}\itemsep0.22em
        ...
        \item RA-clustered SEs: tighten CIs; results survive
        \item RA-level robustness: pop-weighted, lags re-estimated
        \item \textbf{Debt:} CO 2023 excluded (HB23-1126); 60 obs
  ...
  \vspace{0.4em}
  \begin{center}
    \graytext{\small Full tables: \texttt{Analysis/state\_regression\_results.md} \textbar\ \texttt{county\_regression\_results.md}}
  \end{center}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.47\textwidth}
      ...
      \begin{itemize}\itemsep0.32em
        \item \emphcolor{Event study / local projections:} IRFs for \texttt{High\_CDD}, \texttt{High\_HDD}, \texttt{Is\_Extreme\_Drought}, \texttt{High\_AQI\_Max} over $h = 0$--$4$
        \item \emphcolor{County unemployment:} BLS LAUS integration (currently ACS proxy)
        \item \emphcolor{Mechanism decomposition:} respiratory illness vs.\ energy-poverty channels for cold effects
        \item \emphcolor{AQI county models:} extend \texttt{High\_AQI\_Max} shock to premium and debt outcomes
      \end{itemize}
    \end{column}
    \begin{column}{0.47\textwidth}
      \textbf{\tealtext{Writing and Defence}}
      \begin{itemize}\itemsep0.32em
        \item \emphcolor{Dissertation chapter draft:} unified state + county narrative
        \item \emphcolor{Heterogeneity:} rural vs.\ urban; high vs.\ low baseline AQI
        \item \emphcolor{Policy counterfactuals:} ACA Medicaid expansion $\times$ climate shocks
      \end{itemize}
      \vspace{0.5em}
      \tealtext{\small All pipeline code version-controlled;\\ datasets documented in \texttt{CLAUDE.md}.}
    \end{column}
  \end{columns}
\end{frame}
