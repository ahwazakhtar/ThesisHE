  ...
  \begin{frame}[plain]
    \begin{tikzpicture}[remember picture, overlay]
      ...
      \draw[Teal, opacity=0.3, line width=8pt] (current page.south west) ++(-1,-1)
        -- ++(20,0) -- ++(0,12) -- cycle;
      \node[text=white, font=\bfseries\LARGE, align=left,
            anchor=west] at ($(current page.west)+(1.2,0.3)$) {#1};
      \node[text=Teal, font=\normalsize, align=left,
            anchor=west] at ($(current page.west)+(1.2,-0.5)$) {#2};
    \end{tikzpicture}
  \end{frame}%
}

% ── Title slide ───────────────────────────────────────────────────────────────
\title{\textbf{Climate Shocks, Health Finance,\\and Economic Outcomes}}
\subtitle{County-Level Evidence from the United States, 2011–2023}
\author{Thesis Committee Update}
\date{March 2026}
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture, overlay]
    ...
    \draw[WarmOrange, line width=2pt]
      ($(current page.west)+(1.0,0.55)$) -- ++(11,0);
  \end{tikzpicture}
\end{frame}
...
% ── Motivation ────────────────────────────────────────────────────────────────
\begin{frame}{Climate Shocks Create Uninsured Financial Exposure}
  \begin{columns}[T]
...
% ── Descriptive Stats ─────────────────────────────────────────────────────────
\begin{frame}{Key Outcomes: Rising Premiums, Declining Debt Prevalence}
  \begin{center}
    \small
    \begin{tabular}{lrrrc}
      \toprule
...
\begin{frame}{Climate Shock Definitions}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      ...
      \begin{tabular}{lll}
        \toprule
        \textbf{Shock} & \textbf{Definition} & \textbf{Prevalence} \\
        \midrule
        \texttt{High\_CDD} & CDD $\geq$ 1,902 (p80, 1990--00) & 24\% county-yrs \\
        \texttt{High\_HDD} & HDD $\geq$ 5,752 (p80, 1990--00) & 17\% county-yrs \\
        \texttt{Is\_Extreme\_Drought} & PDSI $\leq -4$ & 2.3\% county-yrs \\
        \texttt{High\_AQI\_Max} & Max AQI $> 100$ & 9.2\% county-yrs \\
        \bottomrule
      \end{tabular}
      \vspace{0.6em}
      \textbf{Design choice:} National p80 thresholds from 1990--2000 baseline\\
      \graytext{\footnotesize ``Physically high thermal burden'' regardless of local adaptation norms}
    \end{column}
    \begin{column}{0.40\textwidth}
      \begin{block}{Note for environmental scientists}
        \footnotesize
        CDD/HDD = cumulative annual thermal stress above 65°F / below 65°F base temperature.\\[0.3em]
        PDSI = Palmer Drought Severity Index; $\leq -4$ = ``extreme drought'' by standard AMS classification.\\[0.3em]
        AQI $> 100$ = EPA ``Unhealthy for Sensitive Groups'' threshold.
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Climate Shock Prevalence Over Time}
  \begin{center}
    \includegraphics[width=0.90\textwidth]{../Analysis/plots/descriptive/fig1_climate_shock_prevalence.png}
  \end{center}
...
\begin{frame}{Econometric Model}
  \textbf{Two-Way Fixed Effects with Distributed Lags:}
  \[
    Y_{c,t} = \alpha_c + \gamma_t
    + \sum_{h=0}^{3} \beta_h \, \text{Shock}_{c,t-h}
    + \sum_{h=0}^{2} \delta_h \, \text{Controls}_{c,t-h}
    + \varepsilon_{c,t}
  \]
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \small
      \begin{tabular}{ll}
        $\alpha_c$ & County FE: time-invariant geography \\
        $\gamma_t$ & Year FE: ACA rollout, macro cycles \\
        $h \in \{0,1,2,3\}$ & 0--3 year horizon \\
        $h \in \{-2,-1\}$ & Pre-trend validity check \\
      \end{tabular}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{Two complementary estimators}
        \small
        \textbf{DL}: All lags in one equation\\
        \textbf{LP}: Separate regression per horizon $h$ (Jordà 2005)\\
        \graytext{Results consistent across both}
      \end{block}
    \end{column}
  \end{columns}
  \vspace{0.5em}
  \textbf{Clustering:} State-level (nests rating areas; accounts for spatial climate correlation)\\
  \textbf{Software:} \texttt{fixest::feols} in R
\end{frame}
...
\begin{frame}{Overview: Which Shocks Affect Which Outcomes? ($h=0$)}
  \begin{center}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.34\textwidth}
      ...
      \begin{tabular}{lrl}
        ...
        Silver Premium & --\$2.4 & \graytext{n.s.} \\
        Medical Debt & --0.32 pp & \graytext{n.s.} \\
        Hosp.\ Bad Debt & +\$2.3 & \graytext{n.s.} \\
        \bottomrule
      \end{tabular}
      \vspace{0.5em}
      ...
      \end{itemize}
      \vspace{0.3em}
      \tealtext{Health costs unaffected:\\adaptation via ubiquitous AC?}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Cold vs.\ Heat: A Clear Asymmetry}
  \begin{center}
    \begin{tikzpicture}[scale=0.9,
      ...
      \node[box] (prem) at (4.5, 1.2) {+\$31 premium\\(7\%, p<0.01)};
      \node[box] (bad)  at (4.5, 0.0) {+\$4.43 bad debt\\(p<0.01)};
      \node[box] (debt) at (4.5,-1.2) {+0.29 pp debt\\(p<0.10)};
      \draw[->, WarmOrange, thick] (cold) -- (prem);
      \draw[->, WarmOrange, thick] (cold) -- (bad);
      \draw[->, WarmOrange, thick] (cold) -- (debt);
      % Heat path
      \node[box, fill=Gold!20] (heat) at (0,-3.2) {\texttt{High\_CDD}\\(Extreme Heat)};
      \node[box] (inc) at (4.5,-3.2) {--\$265 HH income\\(0.5\%, p<0.01)};
      \node[box, draw=WarmGray, text=WarmGray] (none) at (4.5,-4.4) {No health cost\\effect};
      \draw[->, DeepRed, thick] (heat) -- (inc);
      \draw[->, WarmGray, dashed] (heat) -- (none);
      % Labels
      \node[font=\scriptsize\itshape, text=WarmGray] at (9.5, 0.0) {Health Finance};
      \node[font=\scriptsize\itshape, text=WarmGray] at (9.5,-3.2) {Income Channel};
    \end{tikzpicture}
  \end{center}
  \vspace{0.2em}
  \begin{center}
...
% ── Drought ───────────────────────────────────────────────────────────────────
\begin{frame}{Drought: Interesting Patterns, Pre-Trend Failure}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      \small
      \begin{tabular}{lrrl}
...
% ── AQI ───────────────────────────────────────────────────────────────────────
\begin{frame}{Air Quality: Promising, Limited by Data Sparsity}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      \small
      \begin{tabular}{lrrl}
...
% ── Robustness Panel ──────────────────────────────────────────────────────────
\begin{frame}{Robustness: DL, LP, and Shock-History Controls Agree}
  \begin{center}
...
% ── Multicollinearity ─────────────────────────────────────────────────────────
\begin{frame}{Multicollinearity: VIFs Well Within Acceptable Range}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \small
      \begin{tabular}{lr}
...
\begin{frame}{Three Policy-Relevant Findings}
  \begin{block}{1. Cold-vulnerable regions carry higher insurance costs}
    Northern counties in extreme cold years face \emphcolor{+7\% premiums} and \emphcolor{elevated hospital bad debt}.\\
    \tealtext{Policy lever:} Targeted winterization investment; cold-weather utilisation risk pools; reinsurance corridors.
  \end{block}
  \vspace{0.3em}
  \begin{block}{2. Heat adaptation appears to be working---but income is eroding}
    No detectable health cost effect from heat, but households lose \emphcolor{$\approx$\$265/year} in income.\\
    \tealtext{Policy lever:} Heat-resilience income supports; cooling assistance (analogous to LIHEAP for heating).
  \end{block}
  \vspace{0.3em}
...
\begin{frame}{Appendix A: Outcome Trends Over Time}
  \begin{center}
    \includegraphics[width=0.88\textwidth]{../Analysis/plots/descriptive/fig2_outcome_index_trends.png}
  \end{center}
\end{frame}
